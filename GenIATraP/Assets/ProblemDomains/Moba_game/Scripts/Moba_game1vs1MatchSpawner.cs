...
{
    public class Moba_game1vs1MatchSpawner : MatchSpawner
    {
        ...
        public override void Respawn<T>(EnvironmentControllerBase environmentController, T respawnComponent)
        {
            ...
            if (Moba_gameEnvironmentController.AgentRespawnType == Moba_gameAgentRespawnType.StartPos)
            {
                respawnPos = agent.StartPosition;
                rotation = agent.StartRotation;
            }
            else if (Moba_gameEnvironmentController.AgentRespawnType == Moba_gameAgentRespawnType.RandomPos)
            {
                GetRandomSpawnPositionAndRotation(Moba_gameEnvironmentController, out respawnPos, out rotation);
            }
            else
            {
