    ...
    public class RayHitObject : ConditionNode
    {
        ...
        protected override bool CheckConditions()
        {
            ...
            if (RAY_HIT_OBJECT_DETECTION_TYPE == RayHitObjectDetectionType.RayIndex)
            {
                ...
                if (sensorPerceiveOutputs[rayIndex].HasHit && sensorPerceiveOutputs[rayIndex].HitGameObjects[0].tag.Contains(TargetGameObjectsToString(targetGameObject)) && TargetTeamHit(sensorPerceiveOutputs[rayIndex].HitGameObjects[0]))
                {
                    targetHit = true;
